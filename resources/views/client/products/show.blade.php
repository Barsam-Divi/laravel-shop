...
@section('content')
    <div id="container">
        <div class="container">
            <!-- Breadcrumb Start-->
            <ul class="breadcrumb">
                <li itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a href="{{route('client.home')}}" itemprop="url"><span itemprop="title"><i class="fa fa-home"></i></span></a></li>
                ...
                <div id="content" class="col-sm-9">
                    <div itemscope itemtype="http://schema.org/محصولات">
                        ...
                        <div class="row product-info">
                            <div class="col-sm-6">
                                <div class="image"><img class="img-responsive" itemprop="image" id="zoom_01" src="{{str_replace('public','/storage',$product->image)}}" title="{{$product->name}}" alt="{{str_replace('public','/storage',$product->image)}}" data-zoom-image="{{str_replace('public','/storage',$product->image)}}" /> </div>
                                ...
                                @if($product->pictures->count() > 0)
                                    <div class="image-additional" id="gallery_01">
                                        ...
                                        <img src="{{str_replace('public','/storage',$product->image)}}" title="{{$product->name}}" alt = "{{str_replace('public','/storage',$product->image)}}"/></a>
                                            @foreach($product->pictures as $pictures)
                                                <a class="thumbnail" href="#" data-zoom-image="{{str_replace('public','/storage',$pictures->path)}}" data-image="{{str_replace('public','/storage',$pictures->path)}}" title="{{$pictures->name}}">
                                                    <img src="{{str_replace('public','/storage',$pictures->path)}}" title="{{$product->name}}" alt = "{{str_replace('public','/storage',$pictures->path)}}"/></a>
                                            @endforeach
                                    </div>
                                @endif
                            </div>
                            <div class="col-sm-6">
                                ...
                                <ul class="price-box">
                                    <li class="price" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
                                        ...
                                        @endif
                                        <span itemprop="price">{{ number_format($product->cost_with_discount).'ریال'}}
                                ...
                                <div id="product">
                                    ...
                                    <div class="cart">
                                        ...
                                        <div>
                                            @auth()
                                                <button type="button" class="wishlist" id="like-{{$product->id}}" onClick="like({{$product->id}})"><i class="fa fa-heart @if(auth()->user()->GetUserLikes($product)) like @endif "></i> افزودن به علاقه مندی ها</button>
                                            @endauth
                                            <br />
                        ...
                        <ul class="nav nav-tabs">
                            ...
                            <li><a href="#tab-review" data-toggle="tab">بررسی ({{count($product->Comments)}})</a></li>
                        </ul>
                        <div class="tab-content">
                            ...
                            <div id="tab-specification" class="tab-pane">
                                @foreach($product->category->propertyGroups as $groups)
                                    <table class="table table-bordered">
                                        <thead>
                                        <tr>
                                            <td colspan="2"><strong>{{$groups->title}}</strong></td>
                                        </tr>
                                        </thead>
                                        <tbody>
                                        <tr>
                                            @foreach($groups->properties as $property)
                                                @if($product->GetProductPropertiesValue($property) !== null)
                                                    <td>{{$property->definition}}</td>
                                                    <td>{{$product->GetProductPropertiesValue($property)}}</td>
                                                @endif
                                            @endforeach
                                        </tr>
                                        </tbody>
                            ...
                            <div id="tab-review" class="tab-pane">
                                        ...
                                        <div>
                                            @foreach($product->Comments as $comment)
                                                <table class="table table-striped table-bordered">
                                                    ...
                                                    <tr>
                                                        <td colspan="2"><p>{{$comment->message}}</p>
                                    ...
                                    @auth()
                                        ...
                                        <form action="{{route('client.products.comments.store' ,$product)}}" method="post">
                                            ...
                                            <div class="form-group required">
                                                <div class="col-sm-12">
                                                    <label for="message" class="control-label">بررسی شما</label>
                                    ...
                                    @else
                                        <h2>برای نوشتن نظر خود لطفا وارد حساب خود شوید</h2>
                                    @endauth
                                    <div class="form-group required">
                                        <div class="col-sm-12">
